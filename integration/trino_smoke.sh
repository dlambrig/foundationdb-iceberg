...
run_and_expect "Delete row" "DELETE FROM iceberg.${SCHEMA}.${TABLE} WHERE order_id = 1" "^DELETE: 1 row|^DELETE: 1 rows$"
run_and_expect "Row count after delete" "SELECT count(*) AS c FROM iceberg.${SCHEMA}.${TABLE}" "^[[:space:]]*2[[:space:]]*$"
run_and_expect "Update row" "UPDATE iceberg.${SCHEMA}.${TABLE} SET note = 'updated by trino' WHERE order_id = 2" "^UPDATE: 1 row|^UPDATE: 1 rows$"
run_and_expect "Read updated row" "SELECT note FROM iceberg.${SCHEMA}.${TABLE} WHERE order_id = 2" "updated by trino"
run_and_expect "Merge rows" "MERGE INTO iceberg.${SCHEMA}.${TABLE} t USING (VALUES (2, 22.5, 'merged existing'), (4, 40.0, 'merged insert')) s(order_id, amount, note) ON (t.order_id = s.order_id) WHEN MATCHED THEN UPDATE SET amount = s.amount, note = s.note WHEN NOT MATCHED THEN INSERT (order_id, amount, note) VALUES (s.order_id, s.amount, s.note)" "^MERGE: 2 rows|^MERGE: 2 row$"
run_and_expect "Read merged rows" "SELECT order_id, amount, note FROM iceberg.${SCHEMA}.${TABLE} ORDER BY order_id" "merged existing|merged insert"
